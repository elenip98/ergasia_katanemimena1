...
<body>
	<jsp:include page="include/header.jsp" />
		...
		<div class="outter">
		<form:form action="stathmoilist" method="post"
						modelAttribute="addremovestathmo">
						...
						<c:forEach var="tempStathmoi" items="${stathmoi}">
							...
							<tr>
								...
								<tr>
									...
									<td><c:if test="${tempStathmoi.enabled == true }">
									<button type="submit" name="RemoveButton" value="${tempStathmoi.id}" >Remove</button></c:if>
									<c:if test="${tempStathmoi.enabled == false }">
									<button  type="submit" name="AddButton" value="${tempStathmoi.id}" >Add</button></c:if></td>
									</tr>
									</table></div>
		</div>
	</c:forEach>
